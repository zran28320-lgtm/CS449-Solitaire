...
class TestManualGame(unittest.TestCase):
    ...
    def test_recording_feature(self):
        self.game.set_recording(True)
        self.game.start_new_game()  # Re-initialize to start record
        self.game.execute_move(3, 1, 3, 3)
        self.game.randomize_board()

        self.assertEqual(len(self.game.history), 3)
        self.assertTrue(self.game.history[0].startswith("INIT"))
        self.assertTrue(self.game.history[1].startswith("MOVE"))
        self.assertTrue(self.game.history[2].startswith("RANDOMIZE"))
